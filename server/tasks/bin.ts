...
type CardBinRow = { id: string; board_id: string; column_id: string | null; title: string; deleted_at: string | null; deleted_by: string | null; purge_started_at: string | null };
...
function cardAccess(cardId: string, userId: string) {
  return db.query(`SELECT k.id, k.board_id, k.column_id, k.title, k.deleted_at, k.deleted_by, k.purge_started_at, b.owner_id, b.name AS board_name, b.deleted_at AS board_deleted_at
    FROM cards k JOIN boards b ON b.id = k.board_id
  ...
  return withBoardLock(boardId, (): TaskRestoreOutcome => {
    ...
    return db.transaction((): TaskRestoreOutcome => {
      ...
      const restored = db.query(`UPDATE cards SET deleted_at = NULL, deleted_by = NULL, purge_after = NULL, column_id = ?, position = ?, updated_at = ?
        WHERE id = ? AND deleted_at IS NOT NULL AND purge_started_at IS NULL`).run(column.id, plan.position, now(), id);
      if (restored.changes !== 1) return { status: "purging" };
      audit(userId, null, "task.card_restore", { boardId: card.board_id, cardId: id, columnId: column.id });
      return { status: "restored", boardId: card.board_id, boardName: card.board_name, columnId: column.id, columnName: column.name };
    })();
  });
}
...
export function purgeTaskLocked(type: TaskBinType, id: string, options: { actorId: string | null; reason: TaskPurgeReason; dueBy?: string }) {
  ...
  return db.transaction(() => {
    ...
    const boardId = type === "board" ? id : boardOfCard(id)!;
    const documentIds = cardAttachmentIds(type === "card" ? { cardId: id } : { boardId: id });
    ...
    audit(options.actorId, null, `task.${type}_purge`, type === "card" ? { boardId, cardId: id, reason: options.reason } : { boardId, reason: options.reason });
    return true;
  })();
}
...
export async function emptyTaskBin(ownerId: string) {
  let purged = 0;
  const cards = db.query("SELECT k.id, k.board_id FROM cards k JOIN boards b ON b.id = k.board_id WHERE b.owner_id = ? AND k.deleted_at IS NOT NULL").all(ownerId) as Array<{ id: string; board_id: string }>;
...
export async function sweepTaskBin(cutoff: string, batchSize: number) {
  let purged = 0;
  const cards = db.query("SELECT id, board_id FROM cards WHERE deleted_at IS NOT NULL AND (purge_started_at IS NOT NULL OR purge_after <= ?) ORDER BY purge_after LIMIT ?").all(cutoff, batchSize) as Array<{ id: string; board_id: string }>;
  for (const card of cards) {
