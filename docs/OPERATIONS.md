...
3. Run `APP_VERSION=0.8.0 GIT_SHA=$(git rev-parse --short HEAD) docker compose up -d --build`.
...
| `APP_VERSION` | `0.8.0` | Build metadata shown in Settings → About and reported by the MCP server. |
