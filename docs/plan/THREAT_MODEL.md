...
| # | Threat | Mitigation | Status |
| --- | --- | --- | --- |
| T115 | **A query or view widens access** (cards from boards the caller cannot read) | `POST /api/tasks/query` ANDs every compiled filter with live cards on live boards the **caller** can read (`readableBoardPredicate`), before sort and limit. Naming another user's board, column, or tag id matches nothing. `has:` subqueries check the other card's board with the same predicate (alias `ob`, derived from the original so they cannot drift). Tested with a three-user matrix, binned cards and boards, and access removed mid-session. | Required |
| T116 | **Metadata leaks through a query** (restricted board, column, or tag names) | `refs` resolves names only for ids on boards the caller can read; everything else is `{id, restricted: true}` with no name, the same for missing and forbidden ids. Users resolve to display names only (the `GET /api/users` exposure). | Required |
