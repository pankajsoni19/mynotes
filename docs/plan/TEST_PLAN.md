...
- [x] `tests/historyDialogs.test.ts`: Back that closes the inner of two stacked dialogs above depth 0 (a colour sheet over Manage tags over a card) pushes no sentinel while the guard's `history.go(1)` is on its way, and the undo lands on the card entry. Found in the 13C browser QA: the sentinel pushed over the board entry cancelled the undo and left the board URL under the open card.
- [x] `tests/tasksCardFace.test.tsx` (card face, §4.4): initials ("Asha Rao" → "AR", one word → one letter, accents kept) and stable avatar tones; the accessible name reads the title, flags, due, every tag, every assignee, and the comment, attachment, related-card, and open-blocker counts in order, and skips a done column's date and tags the board lost; the face (hidden from screen readers) shows flag icons, the excerpt, three tag chips in tagging order with "+N" (titled with the rest), the counts, and three avatars with "+N"; the excerpt replaces the description icon, which stays for a description without an excerpt; an older payload without the Wave 13 fields renders; lane cards are `role="group"` with that name and `aria-describedby` pointing at the excerpt and the key hint, with the Move button outside the face. `tests/tasksCardFields.test.tsx` now expects avatars instead of the first name with "+1".

Manual QA (headless Chrome over CDP at 1280×800 and 390×844 with touch, a scratch `DATA_DIR`, an owner and a member sharing a board, built app):

- [x] Desktop: in the card, typing "Backend" and "Bug" offers **Create “…”**; nothing saves while the list is open, Escape saves both ("Tagged Backend and Bug") and keeps the card open; **Urgent** and **Needs review** save and read `aria-pressed="true"`; **Manage tags…** recolours Bug red through the colour `Select` and renames Backend to "Backend API" with Enter, and the card's chips follow; Back closes only Manage tags, the next Back closes the card.
- [x] Lane card (desktop): flags, a 2-line title, the excerpt, the due chip with time, coloured tag chips, related and open-blocker counts, and initials; its name reads "…, urgent, needs review, due 5 Mar 2027 at 23:00, tags Backend API and Bug, assigned to Asha Owner and Ben Member, 1 related card, blocked by 1 open card", described by the excerpt.
- [x] Phone: no horizontal page scroll; a card due this year with three tags, a flag, an excerpt, and one assignee is 110 px tall with one meta line; the heaviest card (a date in another year, related and blocker counts, two avatars, three tags) wraps its meta row instead of clipping (176 px). The tag list is a bottom sheet with 44 px rows; creating "Temp" there and Back closes only the sheet and saves; flag toggles are 44 px; **Manage tags** is full screen (390×844); the colour sheet over it, then Back closes only the sheet, then deleting Temp through the inline confirm ("It is removed from 1 card.") removes its chip, Back closes Manage tags, Back closes the card, and Forward reopens it.
- [x] Member: no **Manage tags…**; creates "member idea" from the picker and it saves on the card; renaming it through the API is 403; typing "BUG" offers the existing "Bug" and no Create row.
- [ ] Not checked in the browser: a `TAG_EXISTS` race (another person creating the same name meanwhile; the picker then uses theirs, covered by the API tests), the 100-tag board cap message, and screen-reader output itself (the names were read from the DOM).
...
- [x] `tests/tasksComposer.test.tsx` (composer): it starts in the column it was opened from unless that one is full (D108), then the first open column with room, then a done column with room; anything entered except the column makes it dirty; field changes follow the server's rules (clearing the date clears the time, a time needs a date and carries its zone, moving the date keeps the time); assignees keep the names the picker passed, tags and flags replace their sets, and the card-shaped draft feeds `CardFields`; the one create request carries only what was set (relations as `{ targetCardId, type }`, uploads as `attachmentIds`); `COLUMN_FULL` is shown at the column, a 404 at the relations when there are any, the rest above the actions; the composer renders as a modal dialog with the title, a shared column `Select` (no native select), **Create another**, **Create and open**, and **Create** with `aria-keyshortcuts`; a column's **Add a card** is a dialog button, not an inline form (§11 Q4).
- [x] `tests/tasksRoute.test.ts` and `tests/router.test.ts` (full page, §4.7): `/tasks/:b/card/:k/full` parses (uppercase ids and a trailing slash normalise) and formats back; the dialog route carries no `full` key; anything else after the card, a bad card id, or `/full` without a card opens the board; hostile ids never format outside the origin; in-app Back from a deep-linked page replaces page → dialog → board; with the Expand hint Collapse is one step back and Close two (or a replace when the dialog was itself a deep link at depth 0), and without it both replace, so neither leaves Nook.
- [x] `tests/tasksCardPage.test.tsx` (full page): the dialog is modal with a scrim, **Open as page**, and Close; the page layout has no dialog role, `aria-modal`, or scrim, and has **Collapse** instead of Close and Expand; the page frame adds "Back to board <name>" (escaped) and keeps the same element tree as the dialog host, so switching keeps the card view mounted; Expand is hidden at ≤760 px (§11 Q6).

Manual QA (headless Chrome over CDP at 1280×800 and 390×844 with touch, a scratch `DATA_DIR`, two throwaway accounts in separate browser contexts: A owns "Alpha" (shared with B) and "A private"; B owns "Ops" (shared with A)):

- [x] Desktop composer: **New card** opens a 960 px modal with the title focused and no history entry; **Add relation** with **Depends on** finds B's "Order parts" by typing "Order" (the other board's name shows under it) and **Relates to** finds "Secret plan" on "A private"; staged rows are text, not links; a file picked into the hidden input uploads and lists; **Create and open** pushes `/card/:k`, and the new card shows "Depends on" and "Relates to" groups, "Blocked by 1 open card", and the file under Attachments.
- [x] Composer errors and keys: after the owner filled To do behind its back (WIP limit = count), `Ctrl`+`Enter` shows "“To do” is full (limit 2). Move a card out of it first." under the column and keeps the form; picking Doing in the column `Select` and `Ctrl`+`Shift`+`Enter` (with the `Select` focused) creates and empties the form with the title refocused; Enter in the title creates and closes, and focus lands on the new lane card; Escape closes an empty composer at once.
- [x] Phone composer: **Add a card** opens it full screen (390×844, no horizontal scroll) with the depth-0 sentinel; with a title and an upload, Back shows "Discard this card?" ("…the uploaded file moves to your Bin") and stays in Nook; Back again closes only the prompt; **Discard** closes it, and the upload is in A's Bin.
- [x] Relations in the dialog: **Needed by** "Composer card" appears in its group; neither card's revision changes and the other card reads **Depends on** back; the card itself and already-linked cards are not offered; Escape closes only the list; ✕ removes the link.
- [x] Restricted row: B opening A's new card sees "Order parts" as a link and "Restricted card · On a board you cannot open" with a remove button and no link, and neither "Secret plan", "A private", nor its id appears anywhere in B's page.
- [x] Full page (desktop): Expand pushes `/card/:k/full`, the page is not modal (no scrim, no `aria-modal`), the board is hidden, the description and comments sit left of a 360 px fields-and-relations column, and the tab reads "Composer card · Tasks · Nook"; Back returns to the dialog, Forward to the page, **Collapse** to the dialog (title back to "Tasks · Nook"); **Back to board** from a re-expanded page goes to the board; reload on `/full` renders the page; a relation link opens the other board's card and Back returns to the page.
- [x] Full page (phone): the dialog has no Expand; a deep link to `/full` at depth 0 renders the page without horizontal scroll; **Collapse** replaces it with the dialog (depth 0), **Close** replaces that with the board, and the board's in-app Back goes to the board list; from the page, a relation link then Back and Forward step between the two cards.
- [ ] Not checked in the browser: an image pasted into the composer's description (the upload path is the one Attach uses), and `LIMIT_REACHED` or a 404 relation target on create (covered by the unit and API tests).
...
Manual QA (headless Chrome over CDP at 1280×800 and 390×844 with touch, the built app on a scratch `DATA_DIR`, two throwaway accounts sharing a board of 30 cards across three columns with tags, flags, date-only and timed due dates, and two assignees on some):

- [x] Views and history: Columns → Table → List → Calendar each push an entry (`?view=table`, `?view=list`, `?view=calendar`); Back walks back through the views to the lanes and Forward returns; sorting (`sort=due:asc`, `due:desc`, `title:asc`) and **Group by** (`group=assignee`) replace the entry, so Back from the list goes to the table.
- [x] Table: `aria-sort` follows the header; at 390 px the table scrolls sideways inside its region with the title column fixed, and the page never scrolls sideways (the region is positioned so its screen-reader text is clipped with it).
- [x] Filters: **+ Filter** → Flag → Urgent and → Tag → Bug give chips "Flag is Urgent" and "Tag is Bug" and `q=tag:<id>+flag:urgent`; the text box (debounced) adds `"ship"`; Back skips every filter edit; the member opening the same URL sees the same view, grouping, sort, and chips. On the phone the field and value pickers are bottom sheets and the filter bar wraps without horizontal scroll.
- [x] Calendar (desktop): the subtitle says linked events are in Calendar; a timed card (17:30 Europe/Berlin) dragged two days later (synthetic `DragEvent`s) moves to that day, `due_on` shifts by two days, and `due_time`/`due_tz` stay; Alt+← and Alt+↓ move it a day and a week and keep focus; dropping it on **Unscheduled** clears the date and time, and dropping it back on a day schedules it; after the member renamed a card in another browser context, Alt+→ on it rolls back with "Someone else changed this card. It was reloaded." and shows the new title; **Set due date…** saves a date, Escape and Back close it without leaving the view; next/previous month and **Today** replace `month=`, so Back leaves the calendar for the lanes; **Agenda** pushes `cal=agenda`; `q=flag:urgent` filters the grid and the tray.
- [x] Cards keep the query: opening a card from the calendar gives `/card/:k?view=calendar`, **Open as page** gives `/card/:k/full?view=calendar`, Back returns to the dialog, and Escape to the calendar view.
- [x] Calendar (phone): the compact dot grid with the selected day's list below, **Unscheduled (5)** opens a sheet that adds no history entry and Back closes, a day's **Set due date…** dialog closes on Back, and there is no horizontal scroll.
- [x] Calendar module after the refactor: desktop month, phone month, and agenda DOM identical before and after (see `tests/calendarGrid.test.tsx` above); after the merge with main only the Tasks-due counts that the QA itself changed differ.
- [ ] Not checked in the browser: a real pointer drag (synthetic drag events only), a stranger opening a shared filtered URL (the board's existing 404 path), and a screen reader.
...
## Wave 15: viewer and guest enforcement (Team B)

Plan of record: [research/2026-09-26-team-module.md](research/2026-09-26-team-module.md) §10 Team B. No migration. The harness sets `SIGNUP_ROLE=member` so registered test accounts keep writing.

- [x] `tests/guestAudience.test.ts` (T84): one owner shares one item per module with everyone and one with a named guest; a viewer, a guest, and the named guest are checked through `/notes`, `/folders`, `/files`, `/tasks/boards`, `/tasks/views`, `/collections`, `/calendars`, `/events`, `/search`, `/today`, the task query, and every single read (200 or 404); everyone boards list only non-guests as readers; promotion applies at once. The guard test greps `server/**` (not migrations) for `visibility = 'all_users'` not followed by the guest fragment, and the fragment fails closed for unknown users.
- [x] Guest columns in the share matrices: `tests/documents.test.ts` (notes and files: a guest outside every audience and a named guest in the selected ones) and `tests/searchApi.test.ts` (a guest finds nothing shared with everyone).
- [x] `tests/writeGate.test.ts` (T87): every `app.post|put|patch|delete` in `server/` is enumerated (at least 90) and sent as a viewer and as a guest; each is 403 `ROLE_READ_ONLY` unless allowlisted, and allowlisted ones never are; every allowlist entry names a real route; lookalike paths are refused; Team routes keep 404 and `ADMIN_ONLY`; members and admins pass. Editor collections and calendars show `role: "viewer"` to read-only roles, rows and events are refused, the service refuses a demoted owner, the collection query and reminders still work, feed tokens are refused, no Personal calendar is created; a board reader who is a viewer cannot add a card; viewers create, rename, and delete private views but cannot share them, guests cannot create views; guests' task queries need a plain `assignee:me`; `GET /api/users` is refused to read-only roles and carries `role` for others; notifications, preferences, and TOTP setup stay allowed.
- [x] `tests/teamMcp.test.ts` and `tests/mcpScopes.test.ts`: `mcpScopesForRole` per role; a viewer creates read-only keys and is refused write scopes (`SCOPE_NOT_ALLOWED`), a guest is refused (`ROLE_READ_ONLY`) but can revoke; a member's write key demoted to viewer loses the write tools, and as a guest has no tools (`SCOPE_REQUIRED`, empty effective scopes); a viewer's `list_notes` includes an everyone note that the same account cannot read once it is a guest.
- [x] `tests/config.test.ts`: `SIGNUP_ROLE` defaults to guest, accepts guest, viewer, and member, and refuses admin and unknown values. `tests/teamBootstrap.test.ts`: with no active admin the next registration is admin and the one after gets the default guest; the CLI sets viewer.
- [x] `tests/roleChrome.test.tsx` and `tests/teamClient.test.tsx`: `canWriteContent` mirrors the server; `useRole` and the banner ("View only · Team role: Viewer/Guest") follow the provided role and are absent for members and admins; a read-only owner's file sheet has Download but no Rename, Move, Share, or Delete; share hints show Guest and Viewer only; Settings offers MCP permissions matching the server by role and no key form to guests; the role picker has all four roles enabled with the matrix descriptions.
- [x] Manual QA (built app, four accounts, 390 px and 1280 px): the guest sees only the note and board shared by name, none of the everyone note, board, collection, or calendar, gets the banner in each app, no New note, New board, New card, Add a card, upload, or new-folder control, no Team button (Team says it is unavailable), and no MCP key form; the viewer sees the everyone items with the banner, the editor-shared collection as View only, no create controls, and only read permissions in the MCP key form; the admin's role picker lists all four roles and changing one applies; the share picker shows Guest and Viewer hints; Back and Forward move between the board list and a board at 390 px.
...
## Wave 17A: task hierarchy, board structure, and templates

Plan of record: [research/2026-09-26-task-hierarchy-workflows.md](research/2026-09-26-task-hierarchy-workflows.md) §5.1, §6, §7, §11. Migration ids pin 1–17, 019, and 020, and tolerate 018 (Team invites) being absent.

- [x] `tests/migrations.test.ts` (019): on a database that already has 020 but not 017–019: existing cards are level 0 with no parent, sprint, or bin root, and boards are Flat; `structure_json` must be a JSON object of at most 1024 characters; `level` is 0–2; the triggers refuse a parent that skips a level, sits on the same level, is on another board, is unknown, or is the card itself (`PARENT_INVALID`), and a level change on a card with children (`HAS_CHILDREN`); purging a parent detaches its children; `board_sprints` allows one active sprint per board, checks dates and the closed pair, and cascades with its board; deleting a sprint clears `cards.sprint_id`.
- [x] `tests/tasksHierarchy.test.ts` (parent and level): levels default to the parent's plus one, else the work level, and orphans are allowed; an unknown, foreign-board, binned, level-skipping, same-level, or too-deep parent is one 400 `PARENT_INVALID` and writes nothing (T113), and a level past the board's count is `LEVEL_INVALID`; reparent and detach under the revision CAS with `task.card_reparent`; a story cannot sit under a story or itself; Change level refused with `HAS_CHILDREN`, and a new level needs a parent one level up; 100 direct children at most, on create and on reparent (T111); `GET /cards/:k` carries the parent, ancestors (root first), and children by column then position, with roll-up counts; `/children` is 404 for strangers.
- [x] `tests/tasksHierarchy.test.ts` (Bin subtrees): binning an epic bins its five descendants (`descendantCount`, audited); the Bin lists only the root with `descendant_count` for its deleter and the owner and nothing for others; a descendant is 404 on restore and delete forever; one restore brings back the tree with parents, columns, and roll-ups, and clears every `bin_root_id`; a subtask binned on its own keeps its entry, a story binned before the epic keeps its own group, and restoring the epic brings back only its group; a card restored while its parent is binned comes back `detached`; the owner purges a root and its group (the member gets 403), with `descendantCount` in the audit; the sweeper purges roots only, and a separately binned child whose parent was purged restores without a parent. `tests/binFormat.test.ts`: the restore toast names the subitems and the detached case.
- [x] `tests/taskQueryGrammar.test.ts` and `tests/taskQueryParity.test.ts` (filters): `parent:`, `level:`, and `has:subtasks` parse to canonical form and reject bad values, `sprint:` stayed `FILTER_UNSUPPORTED` until 17B; on a three-level board, nine positive and negated hierarchy queries give the same cards through the board's `matchesQuery` and the cross-board query.
- [x] `tests/boardStructure.test.ts` (pure): every preset validates, matches itself, and reads as its label; custom structures; level count, name length, control and bidi characters, extra keys, and work-level bounds; lenient parsing to Flat; level and child names; templates have 2–5 columns, one done column, a todo first column, a valid structure, and no cards.
- [x] `tests/tasksHierarchy.test.ts` (structure): owner-only `PATCH /boards/:b { structure }` (403 members, 404 strangers) with the audit; invalid structures and empty or extra bodies 400; name and structure together; `LEVEL_IN_USE` counts live and binned cards and clears after the purge; a removed level refuses new cards; `SPRINTS_IN_USE` while a sprint is open (T120).
- [x] `tests/hierarchyModel.test.ts` (UI, pure): the columns show the work level and loose lower cards, and everything with Show all levels or on a flat board (D126); roll-ups; children in column-then-position order; ancestors root first; parent candidates one level up; nesting by drag only onto a card one level up that is not already the parent and has room (D128); the checkbox's done and open columns and no checkbox without a done column (D127); the card face label, the Bin confirm, the composer body, and the structure preview; `boardData` recounts roll-ups; the table tree, collapse, and roots whose parent is filtered out; group by parent with No parent last and `group=parent` in the URL; `level:`, `parent:`, and `has:subtasks` over the loaded board, and the Level field only on boards with levels. `tests/boardQuery.test.ts` lists the `parent` grouping.
- [x] `tests/tasksHierarchy.test.ts` (templates): every template creates its columns with states and `is_done`, its structure, and its tags, and no cards; no template is Simple kanban; an unknown one is 400.
- [x] `tests/tasksHierarchy.test.ts` (Today and overlay): an assigned subtask due today carries its parent's title in Today's `tasksDue`, the calendar overlay, and the cross-board query's `parent_title`, and none once detached; `tests/today.test.ts` and `tests/calendarTasks.test.ts` pin `parentTitle: null` on flat cards.
- [x] `tests/mcpTasks.test.ts` (hierarchy): `create_card` with `level` and `parentId` (level names, audit `via: mcp`); `PARENT_INVALID` and `HAS_CHILDREN` as `INVALID` with `reason`; `list_cards` board levels and card hierarchy fields; `get_card` parent title and children; `list_children`, and `NOT_FOUND` for a stranger's key; `update_card` detach and `CARD_CHANGED`; `query_cards` with `level:` and `parent_title`; `list_children` is a `tasks:read` tool.
- [x] `tests/tasksHierarchy.test.ts` (roll-ups): the board payload's `child_count`/`done_child_count` follow moves into and out of the done column, binning a child, and a done column turned off; counts are direct children only; the board and the board list carry `structure`. `tests/tasksBoardPayload.test.ts`: 1000 fully filled cards stay under 1.6 MB with the hierarchy fields (measured 1.50 MB).

## Wave 17B: board sprints

Plan of record: [research/2026-09-26-task-hierarchy-workflows.md](research/2026-09-26-task-hierarchy-workflows.md) §6, §7.3, §7.5, §11. No migration: 17B uses the 019 schema unchanged.

- [x] `tests/tasksSprints.test.ts` (lifecycle): the owner creates sprints with a goal and dates, renames, re-dates, reorders, and starts them, with `task.sprint_create`, `task.sprint_update { fields }`, and `task.sprint_start`; readers list them and strangers get 404; members get 403 `OWNER_ONLY` for every write; end before start, impossible dates, and 61-character names are 400; one active sprint per board (409 `SPRINT_ACTIVE` names it) and it lists first; a board without sprints is `SPRINTS_OFF`; 50 open sprints at most; only an empty planned sprint is deleted (`SPRINT_NOT_EMPTY`, `SPRINT_NOT_PLANNED`); completed sprints page newest first with a cursor, and the board payload carries the open ones plus the last five completed; the Scrum template creates a planned Sprint 1 of two weeks from today, and other templates none.
- [x] `tests/tasksSprints.test.ts` (planning cards): only work-level cards store a sprint (`SPRINT_LEVEL` on create and PATCH, nothing written); subtasks read their parent's sprint and follow it when the parent moves, and leave it when reparented under a backlog task; sprint changes bump the revision under the CAS and are audited; counts are work-level cards only; a sprint of another board or an unknown one is 404, a completed one 409 `SPRINT_COMPLETED`, sprints off 400 `SPRINTS_OFF`, and `null` is always accepted; Change level away from the work level clears the stored sprint.
- [x] `tests/tasksSprints.test.ts` (completing, T118): owner-only (403 members, 404 strangers); carry to the next planned sprint moves the open and in-progress cards with a revision bump, keeps done and binned cards in the completed sprint, moves subtasks by derivation, and leaves other sprints alone, with `task.sprint_complete` counts; a stale editor of a carried card gets `CARD_CHANGED`; only the active sprint completes (`SPRINT_NOT_ACTIVE`), and a completed sprint cannot restart or take cards; carry to the backlog, to a chosen planned sprint, and to a new sprint (Sprint 13, same length, the day after, skipping names the board already uses; or a given name and dates); `NO_NEXT_SPRINT` changes nothing; a sprint of another board, the sprint itself, and unknown targets are 404; a move racing the completion ends consistent either way. `tests/sprintPlan.test.ts`: next names and dates.
- [x] `tests/taskQueryGrammar.test.ts` and `tests/taskQueryParity.test.ts` (the `sprint:` key): `current`, `next`, `none`, `backlog` (read as `none`), and ids parse to canonical form, bad values are `FILTER_INVALID` with the position; on a Scrum board with an active, a next, and a later sprint, subtasks, and loose subtasks, ten positive, negated, and combined sprint queries give the same cards through the board's `matchesQuery` and the cross-board query; across two boards `sprint:current` matches each board's own active sprint, and query cards carry `sprint_id` and `sprint_name`. `tests/tasksQuery.test.ts`, `tests/tasksViews.test.ts`, and `tests/mcpTaskViews.test.ts` now refuse a bad sprint value instead of the reserved key.
- [x] `tests/sprintModel.test.ts` (UI, pure): the board shows the active sprint by default, else the backlog; unknown ids fall back; boards without sprints have no switcher; `?sprint=` round-trips with `view`, drops bad values, is never read as a filter, and rides along to card routes; subtasks take their work-level parent's sprint from the loaded cards; the selection term scopes the board pipeline (All cards adds none), the filter bar's Sprint field and `sprint:current|next` work in memory; progress counts work-level cards by column state; date ranges, timing ("4 days left", "Ends today", "Starts in 4 days"), sprint order, the New sprint and close-dialog defaults, and the composer body. `tests/tasksSprintUi.test.tsx`: the sprint bar (timing, progress, owner-only Complete, Start on a planned sprint, the backlog count), the close dialog (done and unfinished counts, the targets, "Subtasks follow their tasks"), the card's Sprint field (a select on the work level, read-only "(from its task)" below it, hidden without sprints), and Board settings → Sprints for the owner and a member; no native selects.
- [x] Browser QA (390 px and desktop, a scratch instance): a Scrum template board with two sprints; cards planned from the card dialog and the composer (which starts in the sprint on screen); switching sprints from the header and by URL, with Back and Forward replaying the choice; a card opened from a sprint keeps `?sprint=`; Board settings → Sprints adds a sprint with the suggested name and dates; the close dialog and its target sheet close on Back one at a time; completing the active sprint carries the unfinished tasks to the next sprint, keeps the done one, and shows the completed sprint; starting the next sprint makes it the default. No horizontal page scroll at 390 px.
- [x] `tests/mcpTasks.test.ts` (sprints): `list_sprints` in order, by state, and `NOT_FOUND` for a stranger's key; `create_card` with `sprintId` (audited `via: mcp`) and subtasks inheriting it; `SPRINT_LEVEL` as `INVALID` with the reason and an unknown sprint as `NOT_FOUND`; `update_card` moves a task to another sprint with a revision bump and its subtask follows in `get_card`; `list_cards` filters by `current`, `next`, `none`, and an id, and refuses a bad value; `query_cards` with `sprint:next` and `sprint_name`; `list_sprints` is the only sprint tool, in the read scope.

## Wave 17C: Tasks home, My work, and views (UI)

Plan of record: [research/2026-09-26-task-hierarchy-workflows.md](research/2026-09-26-task-hierarchy-workflows.md) §9.4, §9.5, §10, and the §14 Q11 ruling.

- [x] `tests/tasksHome.test.tsx` (routes and codec): `/tasks/my`, `/tasks/views`, `/tasks/views/new`, and `/tasks/views/:id` parse and format (ids lowercased; a malformed id or extra segment opens the list; a board wins over a stray home value); My work writes only what differs from its default (`assignee:me state:todo,doing`, list, grouped by due, sorted by due), "All states" is `?q=assignee:me`, and any link runs as `assignee:me` (negated assignees kept, bad values fall back); a view's override always names layout, group, and sort, round-trips, and compares equal to the saved view it matches; Back steps view → views list → board list → Home; `isSelectiveQuery` refuses empty, text-only, and negated-only filters (Q11).
- [x] `tests/tasksHome.test.tsx` (pure and render): due buckets match the server's grouping; grouping by state, due, assignee (a card in each, "also in"), and tag name across boards; restricted board ids are named "Restricted board" even when the viewer's own list has a name for them (T116); the segments mark the current one; My work shows the state chips with Open checked and a locked "Assignee is Me" chip, and no Save as view at the default; the Boards segment keeps the board list and its heading; results show "2 of 140 cards" and Load more with a cursor, lanes and the table render, and the empty text; the column state field shows To do, In progress, or Done (derived from `is_done` without `state`); view names follow the server rule and Undo re-creates the same body.
- [x] Browser QA (headless Chrome, built app, a scratch `DATA_DIR`, two accounts, four boards with mixed assignees and 125 cards assigned to one account) at 390×844 and 1280×800: My work grouped by due; Load more 50 → 100; table and state lanes with no page scroll sideways; layout switches push and Back/Forward step through them and the segments; state chips push; a Group sheet closes with Back; a view with two filters (two boards, one private, plus `flag:urgent`) created from New view, saved, and shared with everyone; the other account sees it read only, the private board as "Restricted board" and none of its cards, and Duplicate opens a private copy; a card opened from a view is `/tasks/:b/card/:k`, and Back or its close button returns to the view with its rows; Unsaved changes after a group change; a save after the view changed elsewhere offers Reload, Back closes that prompt; Save after a layout change clears the indicator; delete with Undo and Open; Move to… Done from the lanes; the column menu's State select at 390 px writes `state` and `is_done` together; Today's My tasks View all opens `/tasks/my`; document titles per route.
- [ ] Real phone browser: the same segments and view page steps with the system Back gesture.
