...
- [x] `tests/taskQueryGrammar.test.ts` and `tests/taskQueryParity.test.ts` (filters): `parent:`, `level:`, and `has:subtasks` parse to canonical form and reject bad values, `sprint:` stays `FILTER_UNSUPPORTED`; on a three-level board, nine positive and negated hierarchy queries give the same cards through the board's `matchesQuery` and the cross-board query.
