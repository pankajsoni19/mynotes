...
- [x] `tests/boardQuery.test.ts` (one pipeline, D113, §4.5): `boardData` fills the optional payload fields of an older board; every filter field (assignee with `me` and `none`, tag with `none` and an unknown id, flag, column, relations `any`/`blocked`/`none`, the due range and `none`, folded text over the title and excerpt), OR within a field and AND across fields; the relative due buckets on the viewer's day (Monday-first week ending Sunday, a UTC+14 timed card on the UTC−12 viewer's today, a timed card overdue once its instant passed, a date-only card overdue from midnight, buckets OR-ed with the range, and no match without the viewer's clock); each grouping dimension in order, with empty columns kept, a card with two assignees or tags in each group with "also in" labels, and filters applied first; every table sort stable, tie-broken on board order, with empty values last both ways; the filter registry reads and writes the shared filter; and a stub `parent` dimension and filter field plug into the registries without code changes (hierarchy readiness).
