...
1. **No system folder for attachments (revise D42, drop `folders.system_role`).** A hidden folder can be listed, shared, or deleted by its owner; sharing it would leak every attachment. Instead add `documents.purpose TEXT NOT NULL DEFAULT 'file' CHECK (purpose IN ('file','task_attachment','collection_attachment'))` in migration 009 (the third value is reserved for Wave 11 Collections; a SQLite CHECK cannot be widened later without rebuilding the table), store attachments with `folder_id = NULL`, and exclude `purpose <> 'file'` from every Files list, folder count, and the Files Bin filter. Quota still counts them. The uploader sees attachments only on the card.
