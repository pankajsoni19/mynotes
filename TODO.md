...
| 12 Calendar | branch `wave12-calendar` | Stage A done (`7843911`…`d65b871`, 292 tests on the branch: migration 013, recurrence, calendar/event/sharing APIs, agenda/month/event routes, links with a resolver registry, Bin parity, 390 px verified by the implementer). Stage B done (`93ad88f` reminders + dispatcher + notifications API, `ef6c862` bell + `/notifications` + `listUpcoming`; 310 tests on the branch). Stage C (VAPID + payload-less push, service worker, device settings) in progress. Stage D (feeds, MCP tools, docs) runs on main after the merge (needs Wave 8's tool kit). Merge needs a merge agent (overlaps Tasks/Today in router/App/bin wiring). |
...
| 10 Today | `main` | In progress after v0.6.0: migration 011 (due dates, assignees, done columns), `GET /api/today`, Today replaces the Home grid with a launcher row, `get_today` MCP tool. |
...
Merged so far: 9, 8, Bin placement (main `e7f911e`, 343 tests). Next: task tools, 11, 12, then 10. Each gets one independent review; QA is delegated; container verification once per release.
