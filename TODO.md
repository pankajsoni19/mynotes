...
Merged so far: 9, 8, Bin placement, 10, 11 (main `7fd6db2`, 461 tests). Next: Today review fixes → release v0.7.0 (Today + Collections); then Calendar merge + Stage D → v0.8.0; then Collections Stage E (MCP). Each gets one independent review; QA is delegated; container verification once per release.
