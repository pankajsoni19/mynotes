...
import { canManageTeam, canSeeTeam, roleChangeNeedsReauth, ROLES, SELECTABLE_ROLES } from "../src/team/teamRoles";
import { TeamApp } from "../src/team/TeamApp";
import { Select } from "../src/ui/Select";
import { edgeIndex, initialIndex, listKeyAction, stepIndex, typeaheadIndex, type SelectOption } from "../src/ui/selectModel";
...
describe("Select keyboard model", () => {
  ...
  test("arrows skip disabled options and wrap; Home and End go to the ends", () => {
    expect(stepIndex(options, 1, 1)).toBe(4);
    expect(stepIndex(options, 4, 1)).toBe(0);
    expect(stepIndex(options, 0, -1)).toBe(4);
    expect(edgeIndex(options, "first")).toBe(0);
    expect(edgeIndex(options, "last")).toBe(4);
    expect(initialIndex(options, "member")).toBe(1);
    expect(initialIndex(options, "viewer")).toBe(0);
    expect(listKeyAction("ArrowDown", options, 1)).toEqual({ kind: "move", index: 4 });
    expect(listKeyAction("End", options, 0)).toEqual({ kind: "move", index: 4 });
    expect(listKeyAction("Enter", options, 1)).toEqual({ kind: "choose", index: 1 });
    expect(listKeyAction(" ", options, 2)).toEqual({ kind: "none" });
    expect(listKeyAction("Escape", options, 1)).toEqual({ kind: "close" });
    expect(listKeyAction("Tab", options, 1)).toEqual({ kind: "close" });
  });

  test("type-to-search cycles matches and skips disabled ones", () => {
    expect(typeaheadIndex(options, 0, "m")).toBe(1);
    expect(typeaheadIndex(options, 1, "m")).toBe(4);
    expect(typeaheadIndex(options, 4, "m")).toBe(1);
    expect(typeaheadIndex(options, 1, "ma")).toBe(4);
    expect(typeaheadIndex(options, 0, "v")).toBe(-1);
  });

  test("renders a button and a labelled listbox trigger, never a native select", () => {
    const markup = renderToStaticMarkup(<Select label="Team role" value="member" options={options} onChange={() => undefined} hint="Why" />);
