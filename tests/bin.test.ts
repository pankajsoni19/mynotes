...
describe("Bin API", () => {
  test("lists only the caller's binned notes and documents, newest deletion first, with a type filter", async () => {
    ...
    expect(noteItem).toEqual({
      ...
      board_id: null, board_name: null, attachment: false, can_purge: true
    });
    expect(documentItem).toMatchObject({ type: "document", title: "listed.txt", folder_name: "Default", size_bytes: 12, purging: false });
    expect(Object.keys(documentItem!).sort()).toEqual(["attachment", "board_id", "board_name", "can_purge", "deleted_at", "folder_id", "folder_name", "id", "purge_after", "purging", "size_bytes", "title", "type"]);
