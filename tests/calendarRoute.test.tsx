...
test("the app shell accepts the Calendar section and Home shows its card", () => {
  expect(readAppHistorySection(createAppHistoryState("user-1", "calendar", null), "user-1")).toBe("calendar");
  const markup = renderToStaticMarkup(<AppHome displayName="Ada" onOpen={() => undefined} onSettings={() => undefined} onSignOut={() => undefined} />);
  const card = markup.match(/<button class="app-card app-card-calendar">(.*?)<\/button>/)?.[1] ?? "";
  expect(card).toContain("Calendar");
  expect(card).toContain("Plan events and share calendars");
  expect(card).toContain("Open Calendar");
});
