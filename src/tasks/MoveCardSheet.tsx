  ...
  return <ModalDialog title={`Move “${card.title}”`} eyebrow="Move to" onClose={onCancel} variant="sheet" busy={busy}>
    <div className="move-list" role="radiogroup" aria-label="Column">
      {columns.map((column, index) => {
        ...
        return <button key={column.id} role="radio" aria-checked={chosen === column.id} className={`move-option${chosen === column.id ? " active" : ""}`} disabled={busy} autoFocus={index === 0} onClick={() => setChosen(column.id)}>
