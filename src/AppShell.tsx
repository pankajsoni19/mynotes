import { Archive, ArrowRight, FileText, House, Sparkles, Trash2 } from "lucide-react";
import "./appShell.css";
...
export function AppHome({ displayName, onOpen }: ShellProps) {
...
export function AppPlaceholder({ section, onHome, onOpenNotes }: { section: Exclude<AppSection, "home" | "notes">; onHome: () => void; onOpenNotes: () => void }) {
  ...
  return <main className="app-placeholder">
    <header className="app-placeholder-header"><button className="app-home-button" onClick={onHome}><House />Home</button><span className="app-home-brand"><span className="brand-dot"><Sparkles /></span><strong>MyNotes</strong></span></header>
