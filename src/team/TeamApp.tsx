...
import { useHistoryDialog } from "../ui/useHistoryDialog";
...
import { canManageTeam, canSeeTeam, ROLE_DESCRIPTIONS, ROLE_LABELS, ROLES, roleChangeNeedsReauth, SELECTABLE_ROLES, type Role } from "./teamRoles";
import "./team.css";
...
function MemberDetail({ member, members, admin, onBack, onAction, onRoleChosen }: {
  ...
  const roleOptions: SelectOption<Role>[] = ROLES.map((role) => ({
    value: role,
    ...
    <section className="team-card" aria-labelledby="team-role-heading">
      ...
      {admin
        ? <Select
          ...
          hint={lockedByLastAdmin ?? ROLE_DESCRIPTIONS[member.role]}
        />
